...
class SprocketUtils:
    def __init__(self, config:dict, hires:bool, saveworkto:str=None, logger=MockLogger()):
        ...
        self.count = 0
        if '8mm' == self.film:
            self.__waitfunc = partial(SprocketUtils.__findSprocket8mm,self)
        else:
            self.__waitfunc = partial(SprocketUtils.__findSprocketS8,self)

    def __dumpSaved(self) -> None:
        for kk,vv in self.savedwork.items():
            cv2.imwrite(f'/tmp/{self.count}_{kk}.png', vv)

    def __findSprocket8mm(self,image) -> None:
...
#            self.logger.debug(str(picam.capture_metadata()))
            inSprocket = self.__waitfunc(buffer)[0]
